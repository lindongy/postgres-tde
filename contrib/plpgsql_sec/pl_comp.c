...
/*
 * Note on function names: since pl_comp.c uses static variables, we need to
 * ensure that plpgsql_sec does not rely on the core pl_comp.c functions to
 * initialize and update them. Therefore, more function names have the _sec
 * suffix than what might seem necessary.
 *
 * Currently the static variables are datums_alloc and datums_last. Check if
 * some more were added when merging plpgsql_sec into the next PG version.
 */
 ...
 * ----------
 */
static HTAB *plpgsql_sec_HashTable = NULL;
 ...
 * ----------
 */
static PLpgSQL_function *do_compile_sec(FunctionCallInfo fcinfo,
		   HeapTuple procTup,
		   PLpgSQL_function *function,
...
static void plpgsql_start_datums_sec(void);
 ...
 */
PLpgSQL_function *
plpgsql_sec_compile(FunctionCallInfo fcinfo, bool forValidator)
{
...
recheck:
	...
	if (!function_valid)
	{
		 ...
		 * Do the hard part.
		 */
		function = do_compile_sec(fcinfo, procTup, function,
...
/*
 * This is the slow part of plpgsql_sec_compile().
 *
...
do_compile_sec(FunctionCallInfo fcinfo,
			   HeapTuple procTup,
			   PLpgSQL_function *function,
			   PLpgSQL_func_hashkey *hashkey,
			   bool forValidator)
{
	...
	proc_source = plpgsql_decrypt(proc_source);
	...
	switch (function->fn_is_trigger)
	{
		case PLPGSQL_NOT_TRIGGER:
			...
			for (i = 0; i < numargs; i++)
			{
				 ...
				 */
				argvariable = plpgsql_build_variable_sec((argnames &&
														  argnames[i][0] != '\0') ?
														 argnames[i] : buf,
														 0, argdtype, false);
				...
				(num_out_args == 1 && function->fn_prokind == PROKIND_PROCEDURE))
			{
				...
				plpgsql_adddatum_sec((PLpgSQL_datum *) row);
			...
			if (IsPolymorphicType(procStruct->prorettype) &&
				num_out_args == 0)
			{
				(void) plpgsql_build_variable_sec("$0", 0,
		...
		case PLPGSQL_DML_TRIGGER:
			...
			var = plpgsql_build_variable_sec("tg_name", 0,
			...
			var = plpgsql_build_variable_sec("tg_when", 0,
			...
			var = plpgsql_build_variable_sec("tg_level", 0,
											 plpgsql_build_datatype(TEXTOID,
																	-1,
																	function->fn_input_collation,
																	NULL),
			...
			var = plpgsql_build_variable_sec("tg_op", 0,
											 plpgsql_build_datatype(TEXTOID,
																	-1,
																	function->fn_input_collation,
																	NULL),
			...
			var = plpgsql_build_variable_sec("tg_relid", 0,
			...
			var = plpgsql_build_variable_sec("tg_relname", 0,
											 plpgsql_build_datatype(NAMEOID,
																	-1,
																	function->fn_input_collation,
																	NULL),
			...
			var = plpgsql_build_variable_sec("tg_table_name", 0,
											 plpgsql_build_datatype(NAMEOID,
																	-1,
																	function->fn_input_collation,
																	NULL),
			...
			var = plpgsql_build_variable_sec("tg_table_schema", 0,
											 plpgsql_build_datatype(NAMEOID,
																	-1,
																	function->fn_input_collation,
																	NULL),
			...
			var = plpgsql_build_variable_sec("tg_nargs", 0,
			...
			var = plpgsql_build_variable_sec("tg_argv", 0,
		...
		case PLPGSQL_EVENT_TRIGGER:
			...
			var = plpgsql_build_variable_sec("tg_event", 0,
											 plpgsql_build_datatype(TEXTOID,
																	-1,
																	function->fn_input_collation,
																	NULL),
			...
			var = plpgsql_build_variable_sec("tg_tag", 0,
											 plpgsql_build_datatype(TEXTOID,
																	-1,
																	function->fn_input_collation,
																	NULL),
	 ...
	 */
	var = plpgsql_build_variable_sec("found", 0,
 ...
 */
PLpgSQL_function *
plpgsql_sec_compile_inline(char *proc_source)
{
 ...
 *
 * We attempt to recognize the token as a variable only if lookup is true and
 * the plpgsql_IdentifierLookup context permits it.
 *
...
/*
 * plpgsql_build_variable_sec - build a datum-array entry of a given datatype
 *
 ...
 */
PLpgSQL_variable *
plpgsql_build_variable_sec(const char *refname, int lineno,
						   PLpgSQL_type *dtype, bool add2namespace)
{
		...
		case PLPGSQL_TTYPE_SCALAR:
			{
				...
				plpgsql_adddatum_sec((PLpgSQL_datum *) var);
				if (add2namespace)
					 ...
					 bool add2namespace)
{
	...
	rec->firstfield = -1;
	rec->erh = NULL;
	plpgsql_adddatum_sec((PLpgSQL_datum *) rec);
	if (add2namespace)
...
plpgsql_build_recfield(PLpgSQL_rec *rec, const char *fldname)
{
	...
	plpgsql_adddatum_sec((PLpgSQL_datum *) recfield);
 ...
 *
 * origtypname is the parsed form of what the user wrote as the type name.  It
 * can be NULL if the type could not be a composite type, or if it was
...
/* ----------
 * plpgsql_start_datums_sec			Initialize datum list at compile startup.
 * ----------
 */
static void
plpgsql_start_datums_sec(void)
{
	datums_alloc = 128;
	plpgsql_nDatums = 0;
	...
	/* datums_last tracks what's been seen by plpgsql_add_initdatums_sec() */
	datums_last = 0;
}

/* ----------
 * plpgsql_sec_adddatum			Add a variable, record or row
 *					to the compiler's datum list.
 * ----------
 */
void
plpgsql_adddatum_sec(PLpgSQL_datum *newdatum)
{
...
/* ----------
 * plpgsql_add_initdatums_sec		Make an array of the datum numbers of
 ...
 */
int
plpgsql_add_initdatums_sec(int **varnos)
{
	int			i;
	int			n = 0;
...
void
plpgsql_sec_HashTableInit(void)
{
	HASHCTL		ctl;

	/* don't allow double-initialization */
	Assert(plpgsql_sec_HashTable == NULL);

	memset(&ctl, 0, sizeof(ctl));
	ctl.keysize = sizeof(PLpgSQL_func_hashkey);
	ctl.entrysize = sizeof(plpgsql_HashEnt);
	plpgsql_sec_HashTable = hash_create("PLpgSQL function cache",
									FUNCS_PER_USER,
									&ctl,
...
plpgsql_HashTableLookup(PLpgSQL_func_hashkey *func_key)
{
	...
	hentry = (plpgsql_HashEnt *) hash_search(plpgsql_sec_HashTable,
											 (void *) func_key,
											 HASH_FIND,
											 NULL);
	if (hentry)
