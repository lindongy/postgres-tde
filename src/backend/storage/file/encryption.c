...
EnforceLSNForEncryption(char relpersistence, char *buf_contents)
{
	...
	{
		/*
		 * Permanent relation pages usually do have LSN set, but new, empty
		 * pages (or pages created during bulk insert if wal_level is
		 * "minimal") have LSN invalid.
		 *
		 * We must not encrypt multiple pages with the same (invalid) LSN, so
		 * we need to generate some. LSN generated by
		 * GetFakeLSNForUnloggedRel() is not guaranteed to be lower the LSN
		 * assigned to the page in the future. (If the next LSN was lower or
		 * equal to the previous one, recovery would be broken.)
		 */
	...
	{
		/*
		 * For unlogged and temporary tables, there are no restrictions on the
		 * value.
		 */
