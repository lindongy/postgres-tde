...
BufFileCreateFileSet(FileSet *fileset, const char *name)
{
	...
	file->files = (BufFileSegment *) palloc(sizeof(BufFileSegment));
	file->files[0].vfd = MakeNewFileSetSegment(file, 0);
	file->files[0].size = InvalidSegmentSize;
	file->common.readOnly = false;
				   ...
				   bool missing_ok)
{
	...
	for (;;)
	{
		...
		files[nfiles].vfd = FileSetOpen(fileset, segment_name, mode);
	...
	file->files = files;
	file->common.readOnly = (mode == O_RDONLY);
	file->fileset = fileset;
	file->name = pstrdup(name);
...
BufFileSeekTransient(TransientBufFile *file, off_t offset, int whence)
{
	...
	if (file->common.dirty)
	{
		/* Only reached if file->elevel < ERROR */
		ereport(file->elevel,
				(errcode_for_file_access(),
				 errmsg("could not flush file \"%s\": %m", file->path)));
		return EOF;
...
BufFileGetUsefulBytes(File segment, off_t offset, PGAlignedBlock *buffer)
{
	...
	if (FileRead(segment,
		...
		ereport(ERROR,
				(errcode_for_file_access(),
				 errmsg("could not read the page from BufFile \"%s\" at offset %ld: %m",
