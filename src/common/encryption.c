...
run_encryption_key_command(char *data_dir, int *key_len_p)
{
	...
	for (sp = encryption_key_command; *sp; sp++)
	{
		if (*sp == '%')
		{
			if (sp[1] == 'D')
			{
				...
				{
#ifdef FRONTEND
					pg_fatal("data directory is not known, %%D pattern cannot be replaced");
					exit(EXIT_FAILURE);
#else
					ereport(FATAL,
	...
	*dp = '\0';
	...
	{
#ifdef FRONTEND
		pg_fatal("could not execute \"%s\"", cmd);
		exit(EXIT_FAILURE);
#else
		ereport(FATAL,
...
read_encryption_key_f(FILE *f, char *command, int *key_len_p)
{
	...
	while ((c = fgetc(f)) != EOF && c != '\n')
	{
		...
		{
#ifdef FRONTEND
			pg_fatal("encryption key is too long, should contain no more than %d hexadecimal characters",
		...
		{
#ifdef FRONTEND
			pg_fatal("encryption key is too long, should be a %d character hex string",
						 key_len * 2);
			exit(EXIT_FAILURE);
#else
			ereport(FATAL,
...
#ifdef FRONTEND
		pg_fatal("could not read encryption key from %s", src);
		exit(EXIT_FAILURE);
#else
		ereport(FATAL,
	...
	{
#ifdef FRONTEND
		pg_fatal("encryption key is too short, should be a %d character hex string",
					 key_len * 2);
		exit(EXIT_FAILURE);
#else
		ereport(FATAL,
...
encryption_key_from_string(char key_str[ENCRYPTION_KEY_MAX_CHARS], int key_len)
{
	...
	for (i = 0; i < key_len; i++)
	{
		...
		{
#ifdef FRONTEND
			pg_fatal("the encryption key may only contain hexadecimal digits");
			exit(EXIT_FAILURE);
#else
			ereport(FATAL,
