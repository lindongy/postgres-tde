...
write_kdf_file(char *dir)
{
	...
	{
		pg_fatal("KDF directory is too long");
		exit(EXIT_FAILURE);
	}
	...
	if (fd < 0)
	{
		pg_fatal("could not create key derivation file \"%s\": %m", path);
		exit(EXIT_FAILURE);
	}
	...
	if (write(fd, buf, KDF_PARAMS_FILE_SIZE) != KDF_PARAMS_FILE_SIZE)
	{
		...
		if (errno == 0)
			errno = ENOSPC;
		pg_fatal("could not write to key derivation file \"%s\": %m",
					 path);
		exit(EXIT_FAILURE);
	}
	...
	if (close(fd))
	{
		pg_fatal("could not close key setup file: %m");
		exit(EXIT_FAILURE);
	}
}
...
read_kdf_file(char *dir)
{
	...
	if (fd < 0)
	{
		pg_fatal("could not open key setup file \"%s\": %m", path);
		exit(EXIT_FAILURE);
	}
	...
	{
		pg_fatal("could not read from key setup file \"%s\": %m", path);
		exit(EXIT_FAILURE);
	}
	...
	if (version == 0)
	{
		...
		{
			pg_fatal("incorrect checksum in key setup file \"%s\"", path);
			exit(EXIT_FAILURE);
		}
		...
		{
			pg_fatal("unsupported KDF function");
			exit(EXIT_FAILURE);
		}
						 ...
						 int len)
{
	...
	if (rc != 1)
	{
		pg_fatal("failed to derive key from password");
		exit(EXIT_FAILURE);
	}
}
